...
def obj_update(obj_read, o_file):
    ...
    j_synsets =  [i['synsets'] for i in j_obj['objects']]
    synset_write=''
...
def main():
    ...
    o_file = open('obj_list.obj', 'w')
    o_file.close()
    o_file = open('obj_list.obj','a+')
    # Delete all vgm files in current folder:
    
    #pdb.set_trace()
    '''
        sqlite_db = 'object_db.sqlite'
        conn = sqlite3.connect(sqlite_db)
        sqCurs = conn.cursor()
        sqCurs.execute('CREATE TABLE IF NOT EXISTS obj_count(synset TEXT)')
        sqCurs.execute('CREATE INDEX syn_index ON obj_count(synset)')    
